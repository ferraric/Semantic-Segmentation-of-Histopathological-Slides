...
def create_test_data(test_image_folder, test_annotation_folder, output_slices_folder, new_size, file_with_slide_names):
    ...
    for test_image in all_images:
        ...
        valid_tile_pairs = [(image_patch, annotation_patch) for image_patch, annotation_patch in zip(image_tiles, annotation_tiles) if not contains_unknown_label(annotation_patch.image)]
        image_tiles, annotation_tiles = tuple([list(t) for t in zip(*valid_tile_pairs)])
...
def contains_unknown_label(annotation_patch: Image) -> bool:
        pixel_values = np.array(annotation_patch).flatten()
        return np.any(np.isin(pixel_values, [0, 1, 2], invert=True))
