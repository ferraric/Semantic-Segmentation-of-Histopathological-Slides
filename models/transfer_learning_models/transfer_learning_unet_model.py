...
class TransferLearningUnetModel():
    ...
    def build_model(self):
        ...
        if(self.model_type == "unet"):
            ...
            assert self.config.image_size % (self.config.pspnet_downsample_factor*6) == 0, "Image size should be divisible by 32 but is not"
            self.model = sm.FPN(backbone_name=self.config.backbone,
                                ...
                                encoder_weights=self.encoder_weights,
                                encoder_freeze=False,
                                downsaple_factor=self.config.psp_downsample_factor)
