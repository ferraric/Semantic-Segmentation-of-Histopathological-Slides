...
from data_loader.stacked_unet_data_loader import StackedUnetDataLoader, NorwayStackedUnetDataLoader
from models.stacked_unet import StackedUNetModel
...
def main():
    ...
    model = StackedUNetModel(config)
    if (config.norway_dataset):
        print("using norwegian dataset")
        assert config.number_of_classes == 2, config.number_of_classes
        train_dataloader = NorwayStackedUnetDataLoader(config,
                                                       validation=False,
                                                       preprocessing=lambda im: im / 255.0)
        validation_dataloader = NorwayStackedUnetDataLoader(config,
                                                            validation=True,
                                                            preprocessing=lambda im: im / 255.0)
    else:
        print("using our dataset")
        train_dataloader = StackedUnetDataLoader(config,
                                                 validation=False,
                                                 preprocessing=lambda im: im / 255.0)
        validation_dataloader = StackedUnetDataLoader(config,
                                                      validation=True,
                                                      preprocessing=lambda im: im / 255.0)

    # print the model summary and save it into the output folder
    dummy_model = StackedUNetModel(config)
...
def save_input_label_and_prediction(model, validation_dataloader, comet_experiment, config, epoch, step):
    for i, data in enumerate(validation_dataloader.dataset):
        ...
        assert data[1][0].numpy().shape == (config.image_size, config.image_size, config.number_of_classes), data[1][
            0].numpy().shape
...
class EvaluateDuringTraningCallback(tf.keras.callbacks.Callback):
    ...
    def on_batch_end(self, batch, logs=None):
        ...
        if (self.seen % self.validate_every_n_steps == 0 and self.seen != 0):
            ...
            evaluation_metrics = self.model.evaluate(self.validation_dataloader.dataset,
                                                     steps=len(self.validation_dataloader))
            print("Evaluation metrics", evaluation_metrics)
            for i in range(1, len(evaluation_metrics)):
                self.comet_experiment.log_metric("callback_validation" + self.model.metrics[i - 1].name,
