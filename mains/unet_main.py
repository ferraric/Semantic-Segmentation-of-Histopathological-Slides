...
from data_loader.unet_data_loader import UnetDataLoader, NorwayUnetDataLoader
...
def main():
    ...
    if (config.norway_dataset):
        ...
        train_dataloader = NorwayUnetDataLoader(config,
                                                validation=False,
                                                preprocessing=lambda im: im / 255.0,
                                                use_image_augmentations=config.use_image_augmentations)
        validation_dataloader = NorwayUnetDataLoader(config,
                                                     validation=True,
                                                     preprocessing=lambda im: im / 255.0)
    else:
        print("using our dataset")
        train_dataloader = UnetDataLoader(config,
                                          validation=False,
                                          preprocessing=lambda im: im / 255.0,
                                          use_image_augmentations=config.use_image_augmentations)
        validation_dataloader = UnetDataLoader(config,
                                               validation=True,
                                               preprocessing=lambda im: im / 255.0)
